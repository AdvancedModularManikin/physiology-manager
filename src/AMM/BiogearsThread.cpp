	...
	bool BiogearsThread::LoadScenarioFile(const std::string &scenarioFile) {
		...
		if (scenarioLoading) {
			if (sce.HasEngineStateFile()) {
				...
				biogears::SEScenarioInitialParameters &sip = sce.GetInitialParameters();
				if (sip.HasPatientFile()) {
					std::vector<const biogears::SECondition *> conditions;
					for (biogears::SECondition *c: sip.GetConditions())
		...
		for (biogears::SEAction *a: sce.GetActions()) {
			...
			if (adv != nullptr) {
				...
				try {
					...
					LOG_INFO << "Simulating " << adv->GetTime(biogears::TimeUnit::s) << " seconds...";
					m_pe->AdvanceModelTime(adv->GetTime(biogears::TimeUnit::s), biogears::TimeUnit::s);
					auto end = std::chrono::high_resolution_clock::now();
					auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin);
					LOG_INFO << "Done simulating time advancement. Simulated " << adv->GetTime(biogears::TimeUnit::s)
					         << "s in "
		...
		for (auto str: strings) {
			...
			try {
				if (kvp_k == "type") {
					...
					LOG_WARNING << "Unknown pump setting: " << kvp_k << " = " << kvp_v;
				}
			} catch (std::exception &e) {
		...
		try {
			if (type == "infusion") {
				...
				if (substance == "Saline" || substance == "Whole Blood" || substance == "WholeBlood" ||
				    substance == "Antibiotic" || substance == "Blood" || substance == "RingersLactate" ||
				    substance == "PRBC") {
		...
		for (auto str: strings) {
			...
			try {
				if (kvp_k == "OxygenFraction") {
					...
					LOG_WARNING << "Unknown ventilator setting: " << kvp_k << " = " << kvp_v;
				}
			} catch (std::exception &e) {
				LOG_ERROR << "Issue with setting " << e.what();
			}
		...
		for (auto str: strings) {
			...
			try {
				if (kvp_k == "OxygenFraction") {
					...
					LOG_WARNING << "Unknown BVM setting: " << kvp_k << " = " << kvp_v;
				}
			} catch (std::exception &e) {
				LOG_ERROR << "Issue with setting " << e.what();
			}
 ...
 */
	...
	double BiogearsThread::GetCerebralPerfusionPressure() {
		return m_pe->GetCardiovascularSystem()->GetCerebralPerfusionPressure(biogears::PressureUnit::mmHg);
	}

	double BiogearsThread::GetCerebralBloodFlow() {
		return m_pe->GetCardiovascularSystem()->GetCerebralBloodFlow(biogears::VolumePerTimeUnit::mL_Per_min);
	}

	double BiogearsThread::GetPatientAge() {
		return m_pe->GetPatient().GetAge(biogears::TimeUnit::yr);
	}

	double BiogearsThread::GetPatientWeight() {
		return m_pe->GetPatient().GetWeight(biogears::MassUnit::kg);
	}

	double BiogearsThread::GetPatientGender() {
		// return m_pe->GetPatient().GetGender();
		return 0;
	}

	double BiogearsThread::GetPatientHeight() {
		return m_pe->GetPatient().GetHeight(biogears::LengthUnit::cm);
	}
